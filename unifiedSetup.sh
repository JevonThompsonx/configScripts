...
clone_user_configs() {
  ...
  cd "$HOME" || {
    echo "⚠️  Could not change to HOME directory. Skipping config cloning."
    return 1
  }
...
setup_arch() {
  ...
  local packages=(
    ...
    xsel xclip appimagelauncher clamav wezterm-nightly-bin # <-- ADDED ClamAV for malware scanning
  )
